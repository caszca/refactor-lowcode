...
export function getTemplateFile(keywords, suffix = '') {
  switch (keywords) {
    ...
    case 'registerTaskCheck':
      return { name: '报到登记查看' + suffix, id: null }
    case 'TaskStatic':
      return { name: '报到登记统计' + suffix, id: null }
    case 'RelTeacherAssessTaskUser1':
      return { name: '学院考核评分批量录入' + suffix, id: 8173 }
    case 'RelTeacherAssessTaskUser':
      return { name: '考核结果导出' + suffix, id: null }
...
export function customize(obj, src) {
  var ret = _.cloneDeep(_customize(obj, src))
  return ret
}

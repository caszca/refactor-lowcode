...
export default {
  ...
  props: {
    defaultProps: { // defaultProps每一项和默认值无用，写清楚方便后面查询调用
      type: Object,
      default: () => {
        return {
    ...
    selectedColumns: { type: Array, default: () => { return [] } }
  },
  data() {
    return {
      fileList: [],
  ...
  async created() {
    ...
    if (this.defaultProps.searchPanel === false) {
      this.$set(this, 'showSearchPanel', false)
    } else { this.$set(this, 'showSearchPanel', true) }
  ...
  methods: {
    ...
    async handleUpload(para) {
      // console.log(this.$parent.initSearchWords)

<template>
  <div>
    <el-select v-model="selectValue" v-loading="loading" class="simple-select" :placeholder="placeholder" :size="size" :multiple="multiple" :disabled="disabled" :filterable="filterable" clearable @change="handleChange">
      <el-option v-for="item in options" :key="item.id + selectValue" :label="item.name" :value="item.id" :value-key="item.id" />
    </el-select>
  </div>
</template>

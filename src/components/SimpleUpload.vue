<template>
  <div>
    <el-upload ref="upload" v-model="fileList" action multiple :limit="limit" :auto-upload="autoUpload" :before-remove="handleBeforeRemove" :on-preview="handlePreview" :on-remove="handleRemove" :on-change="handleChange" :file-list="fileList" :http-request="() => {}" :before-upload="beforeUploadVerify">
...
// import img from '@/assets/icons/img/pdficon.jpeg'
// import doc from '@/assets/icons/img/doc.png'
...
export default {
  ...
  watch: {
    fileList: {
      handler(n, o) {
        this.$emit('updateFileList', n)
      },
      deep: true
    }
  },
  mounted() {
    console.log(this.upButtonInfo)
  ...
  methods: {
    ...
    beforeUploadVerify(file) {
      ...
      if ((this.fileAllowedTypes.length > 0) && (this.fileAllowedTypes.indexOf(fileFormat) === -1)) {
    ...
    async handleRemove(file, fileList) {
      if (file.status === 'success') {
        await fileAPI.deleteFile([file.uid])
      }
      this.fileList = fileList
    },
    async handleUpload(tabName, userId, relId) {
      const files = []
      this.fileList.forEach(fi => {
        files.push(fi.raw)
      })
      ...
      if (file.status === 'success') {
        this.$confirm('是否下载该文件？', '提示')
          .then(async _ => {
            ...
            const content = await fileAPI.downloadFile(file.uid)
